...
import { TextSprite } from 'three-text2d';
...
function createTextSprite(text, color) {
  ...
  sprite.position.set(Math.random()*2,Math.random()*2,Math.random()*2)
  sprite.scale.set(2, 1, 1);

  return sprite;
}

function addPlayerName(player, car,scene, renderer,controls) {
  const playerName = player.state.profile.name; // Replace this with the actual method to get player name
  const textSprite = createTextSprite(playerName, 'white');
  textSprite.position.set(0, 2, 0); // Adjust the position based on your car model
  // car.container.add(textSprite);

  
  const updateTextPosition = () => {
    textSprite.position.copy(car.container.position);
    textSprite.position.y += 2; // Adjust the height offset as needed
  };
}
...
function setupGame() {
  ...
  onPlayerJoin(async (player) => {
    ...
    addPlayerName(player, car,scene,renderer,controls);
